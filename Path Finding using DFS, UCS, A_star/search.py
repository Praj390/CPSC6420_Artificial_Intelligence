    ...
    while open_set:
        ...
        for i in ACTIONS:                                   #Finding children of node
            ...
            if child not in open_set and child not in closed_set:                                   #Checking the children in open set and closed set, if not then find children
                if child not in obstacles and 0 <= child[0] < n_rows and 0 <= child[1] < n_cols:        #Entering boundary condition and obstacle check for child
                    open_set.add(child)
                    parent[child[0]][child[1]] = (node[0],node[1])                                      #Placing the parent of respective child
    path.reverse()                                                                #Because of backtracing we have to reverse the path
    path.append(goal)


    for i in range(len(path)-1):                                                   #Finding the movement, substracting the parent from child
...
def uniform_cost_search(grid_size, start, goal, obstacles, costFn, logger):
    ...
    while open_set:
        ...
        for i in ACTIONS:
            child = (node[0] + i[0], node[1] + i[1])
            f_cost = cost + costFn(child)

            if child not in open_set and child not in closed_set:
                if child not in obstacles and 0 <= child[0] < n_rows and 0 <= child[1] < n_cols:
