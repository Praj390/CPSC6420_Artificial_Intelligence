...
class ExactInference(Inference):
    ...
    def observe(self,
        ...
        while column < num_cols:
            row = 0
            # r,c = self.get_coordinate(row,column)
            while row < num_rows:
                r,c = self.get_coordinate(row,column)
                prev_prob = self.belief[row][column]
                pdf = 1
                # for four landmarks
                for i in range(4):
                    agent_x = landmarks[i][0]
                    agent_y = landmarks[i][1]
                    distance = math.sqrt((agent_y-c)**2 + (agent_x - r)**2)
                    pdf *= normal_pdf(distance,App.SENSOR_NOISE,observed_distances[i])

                new_prob = (row,column,prev_prob*pdf)
    ...
    def timeUpdate(self):
        ...
        for r in range(num_rows):
            for c in range(num_cols):
                post_prob = self.belief[r][c]
                transition = self.transition_model(r,c)
                for i in transition:
                    transition_probability = i[0]
                    r_,c_ = i[1]
                    res_prob = post_prob*transition_probability
                    new_prob = (r_,c_,res_prob)
                    new_probs.append(new_prob)
...
class ParticleFilter(Inference):
    ...
    def observe(self,
        ...
        1. Compute the weight w_i of each particle i in self.particles as w_i = p(e_t|x_i),
        ...
        for i in self.particles:
            ...
            for j in range(4):
                agent_x = landmarks[j][0]
                agent_y = landmarks[j][1]
                distance = math.sqrt((agent_y-c)**2 + (agent_x - r)**2)
                pdf *= normal_pdf(distance,App.SENSOR_NOISE,observed_distances[j])
            weights.append(pdf)
        old_particles = self.particles[:]
        for i in range(self.NUM_PARTICLES):
            self.particles[i] = (random.choices(old_particles,weights = weights, k=1))[0]
        self.updateBelief()
    ...
    def timeUpdate(self):
        ...
        for i in range(len(self.particles)):
            transition = self.transition_model(self.particles[i][0],self.particles[i][1])
            weights = np.zeros(len(transition))
            for j in range(len(transition)):
                weights[j] = transition[j][0]
            update_particle = random.choices(transition, weights =weights, k=1)
            # print(update_particle)
            self.particles[i] = update_particle[0][1]
        self.updateBelief()
