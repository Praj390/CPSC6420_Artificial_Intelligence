    ...
    for k in range(max_iterations):
        ...
        for s in range(NUM_STATES):
            ...
            for a in range(NUM_ACTIONS):
                for p, s_, r, terminal in env.trans_model[s][a]:
                    if terminal:
                        q[a] += p * r
                    else:
                        q[a] += p * (r + gamma * v_prev[s_])
...
###############################################################################
    ...
    for k in range(max_iterations):
        for i in range(max_iterations):
            ...
            for s in range(NUM_STATES):
                temp = 0
                for p, s_, r, terminal in env.trans_model[s][pi[s]]:
                    if terminal:
                        temp += p*r
                    else:
                        temp += p*(r+gamma*v_prev[s_])
                delta = max(delta, abs(v_prev[s]-temp))
                v[s] = temp
            ...
            for a in range(NUM_ACTIONS):
                for p, s_, r, terminal in env.trans_model[s][a]:
                    if terminal:
                        ...
                        q[a] += p * (r + gamma * v[s_])
            best_policy = np.argmax(q)

            if old_a != best_policy:
                policy_stable = False
