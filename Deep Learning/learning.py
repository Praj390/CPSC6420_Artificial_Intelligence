...
def linear_regression(x, y, logger=None):
    ...
    for i in range(max_iteration):
        h = np.dot(x,w)
        loss  = np.square(np.subtract(h,y))*0.5
        obj_fun = np.dot(np.subtract(h,y),x)
        w = w - alpha * obj_fun
        if i % 10 == 0:
            logger.log(i,sum(loss))
    return w
...
def binary_perceptron(x, y, logger=None):
    ...
    for i in range(max_iteration):
        product = np.dot(x,w)
        h = np.where(product>=0,1,0)
        diff = np.subtract(y,h)
        w = w + np.dot(diff,x)
    return w
...
def logistic_regression(x, y, logger=None):
    ...
    for i in range(max_iteration):
        ...
        w = w - alpha*np.dot(diff,x)
        cost = np.sum(np.dot(y,np.log(h)) + np.dot(np.subtract(1,y),np.log(np.subtract(1,h)))) * -1
        if i % 10 == 0:
            logger.log(i,cost)
    return w
